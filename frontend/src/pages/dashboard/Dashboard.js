...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    // If in demo mode, use our demo data instead of fetching from API
    if (isInDemoMode()) {
      setStats(DEMO_STATS);
  ...
  const todayAppointments = appointments ? appointments.filter((appointment) => {
  ...
  // Filter appointments if we're not in demo mode
  if (!isInDemoMode() && appointments) {
    const filteredAppointments = appointments.filter((appointment) => {
      if (!appointment.date) return false;
      const appointmentDate = new Date(appointment.date);
      return appointmentDate > today && appointmentDate <= nextWeek;
    });
    setUpcomingAppointmentsList(filteredAppointments);
  }
  ...
  const cards = [
    {
      ...
      value: todayAppointments.length || stats.appointments.today,
      href: '/appointments',
      icon: CalendarIcon,
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    <div className={`h-10 w-10 rounded-full flex items-center justify-center ${
                      appointment.isVirtual ? 'bg-indigo-100 text-indigo-500' : 'bg-blue-100 text-blue-500'
                    }`}>
